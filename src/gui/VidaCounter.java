 ...
 *
 */
public class VidaCounter extends javax.swing.JPanel {
    private static final Punto POSICION = new Punto(10, 20);
    private static final int SEPARACION_Y = 20;
    private static final int SEPARACION_X = 20;
    ...
    protected void paintComponent(final Graphics g) {
        ...
        for (Pacman p : this.escenario.getPacmans()) {
            int y = POSICION.getY() + (i++) * SEPARACION_Y;
            
            for (int v = 0; v < p.getVidas(); v++) {
                int x = POSICION.getX() + v * SEPARACION_X;
                g.drawImage(p.getDefaultImage(), x, y, this);
            }
        }
    }
